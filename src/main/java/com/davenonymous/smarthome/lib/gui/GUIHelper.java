...
public class GUIHelper {
	public static ResourceLocation tabIcons = SmartHome.resource("textures/gui/tabicons.png");
	private static Map<Integer, DynamicImageResources.DynTexture> circleCache = new HashMap<>();

	public static void drawCheckerBoard(GuiGraphics guiGraphics, int x, int y, int width, int height, int color1, int color2) {
		...
		for(int i = 0; i < width; i += 8) {
			for(int j = 0; j < height; j += 8) {
				int color = ((i / 8 + j / 8) % 2 == 0) ? color1 : color2;
				bufferbuilder.addVertex(matrix4f, x + i, y + j + 8, 0f).setColor(color);
				bufferbuilder.addVertex(matrix4f, x + i + 8, y + j + 8, 0f).setColor(color);
				bufferbuilder.addVertex(matrix4f, x + i + 8, y + j, 0f).setColor(color);
				bufferbuilder.addVertex(matrix4f, x + i, y + j, 0f).setColor(color);
			}
		}
	...
	public static void drawArrowHead(GuiGraphics guiGraphics, float x1, float y1, float x2, float y2, int color) {
		...
		ResourceLocation arrowHeadTexture = SmartHome.resource("textures/gui/arrowhead.png");
		RenderSystem.setShader(GameRenderer::getPositionTexColorShader);
